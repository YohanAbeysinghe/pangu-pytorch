...
def Exloss(pred, target, up_th=0.9, down_th=0.1, lamda_underestimate=1.2, lamda_overestimate=1.0, lamda=1.0):
    ...
    tar_up =  torch.quantile(target.reshape(N, C, H*W), q=up_th, dim=-1).unsqueeze(-1).unsqueeze(-1) # N,C,1,1
    tar_down =  torch.quantile(target.reshape(N, C, H*W), q=down_th, dim=-1).unsqueeze(-1).unsqueeze(-1) # N,C,1,1
...
if __name__ == "__main__":
    ...
    channel_weights = [1.50, 0.77, 0.66, 3.00, 1.20]
    # pred = torch.randn(1,69,721,1440)
    # target = torch.randn(1,69,721,1440)
    # print(Exloss(pred, target))

    pred = torch.randn(1, 5, 13, 209, 306)
    target = torch.randn(1, 5, 13, 209, 306)

    weighted_upper_loss = 0

    for i in range(5):
        w = float(channel_weights[i])
        pred_slice = pred[:, i, :, :, :]    # (N, C, H, W)
        target_slice = target[:, i, :, :, :]  # (N, C, H, W)
        loss = Exloss(pred_slice, target_slice)
        weighted_upper_loss += w * loss

    print(weighted_upper_loss)