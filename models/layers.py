  ...
  def forward(self, x, Z, H, W):
    ...
    for blk in self.blocks:
      ...
      if self.use_checkpoint:
        if i % 2 == 0:
          x = checkpoint.checkpoint(blk, x, Z, H, W, False, use_reentrant=False)# @Yohan. ", use_reentrant=False" added to avoid a warning.
        else:
          x = checkpoint.checkpoint(blk, x, Z, H, W, True, use_reentrant=False)# @Yohan
      else:
          if i % 2 == 0:
...
class EarthSpecificBlock(nn.Module):
  ...
  def forward(self, x, Z, H, W, roll):
    ...
    x = x + self.drop_path(self.norm2(self.linear(x)))  # [1, 521280, 192] Current [1, 20640, 192]
