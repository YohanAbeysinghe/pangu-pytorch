...
def weighted_rmse_cropped(pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    ...
    weighted_diff_sq = weight * diff_sq
    mean_weighted_diff_sq = torch.mean(weighted_diff_sq, dim=(-1, -2))

    # ✅ Clamp to avoid NaNs from sqrt of negative numbers due to floating point error
    mean_weighted_diff_sq = torch.clamp(mean_weighted_diff_sq, min=0.0)
    result = torch.sqrt(mean_weighted_diff_sq)
